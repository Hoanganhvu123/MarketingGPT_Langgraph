...
class Writer:
    def __init__(self, llm, prompt_template: str, verbose=False, **kwargs):
        self.llm = llm
        self.verbose = verbose
        self.prompt = PromptTemplate(template=prompt_template, input_variables=["search_result"])
        self.chain = LLMChain(llm=self.llm, prompt=self.prompt, verbose=self.verbose)
...
if __name__ == "__main__":
    state = {"search_result": "This is a test search result."}
